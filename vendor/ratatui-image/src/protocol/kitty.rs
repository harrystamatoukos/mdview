...
fn transmit_virtual(img: &DynamicImage, id: u32, is_tmux: bool) -> String {
    ...
    // reader transmits large per-band images (~8M px ≈ 32 MB raw); the transmit
    // string is embedded in a ratatui buffer cell and thus flushed synchronously
    // by the UI thread, so raw RGBA costs ~650 ms per band and dominates scroll
    ...
    // loop (+4 for the optional `o=z,` key)
    ...
    for (i, chunk) in chunks.enumerate() {
        ...
        if i == 0 {
            // mdview patch: `o=z` marks the pixel data as zlib-compressed; s/v
            // stay the *uncompressed* pixel dimensions.
            let o = if zlib { "o=z," } else { "" };
            write!(data, "i={id},a=T,U=1,f=32,t=d,{o}s={w},v={h},").unwrap();
        }
